...
def get_mini_batch(x_val, f_x_val, b):
	last = 0
	counter = 0
	while True:
...
class LogisticRegression:
	def __init__(self, grad='batch', weights=None, alpha=0.0001,
				 n_cycle=1000000, b=18):
		self.grad = grad
		self.weights = weights
		self.alpha = alpha
		self.n_cycle = n_cycle
		self.b = b  # b == mini-batch size,  2 =< b =< 32, here 18
	...
	def mini_batch(self, x_val, f_x_val, debug):
		for x, y, _ in get_mini_batch(x_val, f_x_val, self.b):
			if _ > self.n_cycle:
				break
	...
	def loss(self, predicted_values, expected_values):
